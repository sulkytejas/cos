/**
 * Stubbed Calendar connector. Same shape as Gmail. Real OAuth in v0.3.
 */
...
export async function pollOnce(): Promise<void> {
  ...
  for (const fx of fixtures) {
    const existing = db
      ...
      .from(schema.signals)
      .where(
        and(eq(schema.signals.source, "calendar"), eq(schema.signals.externalId, fx.external_id))
      )
      .get();
  ...
  if (inserted > 0) console.log(`[calendar] ingested ${inserted} signals`);
}
